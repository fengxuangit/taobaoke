...
<li class="{if $_GET.status ==''}select{/if}"><a href="{$URL}m=money&a=tixian">全&nbsp;&nbsp;部</a></li>
{foreach from=$_G.setting.tixian_status item=v key=k}
<li class="{if $_GET.status != ''&& $_GET.status==$k}on{/if}"> <a href="{$URL}m=money&a=tixian&status={$k}">{$v}</a></li>
{/foreach}
        </ul>
  </div>  
...
<td>	<a href="{$URL}m=money&a=tixian&uid={$_G.uid}">{$v.username}</a></td>    
<td>{$v.money}</td>
<td>{$v.shouxufei}</td>
...
<td><a href="{$URL}m=money&a=post_tixian&id={$v.id}" >编辑</a></td>
     

<td><a href="{$URL}m=money&a=del_tixian&id={$v.id}&ok=1" class="_confirm" data-msg="您确认删除当前记录吗?删险后不可恢复">删除</a></td>
