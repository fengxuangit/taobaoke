...
<li class="{if $_GET.status ==''}select{/if}"><a href="{$URL}m=fanli&a=money">全&nbsp;&nbsp;部</a></li>
{foreach from=$_G.setting.money_status item=v key=k}
<li class="{if $_GET.status != ''&& $_GET.status==$k}on{/if}"> <a href="{$URL}m=fanli&a=money&status={$k}">{$v}</a></li>
{/foreach}
        </ul>
  </div>  
...
<td>	<a href="{$URL}m=fanli&a=money&uid={$v.uid}">{$v.username}</a></td>    
<td>{$v.money}</td>
<td>{$v.org_money}</td>
<td>{$v.status_text}</td>
<td>{$v.desc}</td>
...
<td><a href="{$URL}m=fanli&a=del_money&id={$v.id}&ok=1" class="_confirm" data-msg="您确认删除当前记录吗?删险后不可恢复">删除</a></td>
